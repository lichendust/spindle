...
%{link docs/style.css}</code></pre><p>Links that begin with a protocol, like <code>http:</code> or <code>https:</code>, are passed through, so you can use templating to reference</p><p>Index pages are implicitly understood, however, and are simply accessed by supplying the directory path, though <code>dir/index</code> will also work.</p><p>By default, Locators expand to the URL-style specified in <code>spindle.toml</code>, such as <code>absolute</code>, <code>relative</code> or <code>root</code>.</p><p>However, each *individual* Locator can also be overriden as a one-off.</p><pre><code>%{link:abs image.jpg}
