...
Spindle makes use of a piece of syntax called a ['Resource Finder'](%{page syntax}#resource-finder): this streamlines the process of linking between pages — you can just put in the page's name, not its full path — and the resulting link styles can all be changed according to the needs of the site in `spindle.toml` (absolute, relative, etc.).
